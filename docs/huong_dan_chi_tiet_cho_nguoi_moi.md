...
Các số dòng dưới đây được viết theo bản đầu tiên của `src/main.cpp`; sau bản vá kỹ thuật trong `docs/phan_tich_sau_analyze_va_de_xuat.md`, một số số dòng có thể lệch nhẹ. Khi đọc, hãy ưu tiên tên hàm và dùng `Ctrl+F` theo tên hàm; các mốc dòng mới nhất của bản vá nằm trong tài liệu phân tích sau analyze. Những dòng chỉ là `{`, `}`, dòng trắng hoặc dấu chấm phẩy kết thúc cấu trúc sẽ được gom vào cụm dòng vì chúng không mang logic riêng.
